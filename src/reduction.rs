...
use crate::core::{
    ValueType, IndexType, CoefficientType, Entry, DiameterEntry,
...
impl SparseColumn {
    ...
    pub fn multiply_by_scalar(&mut self, scalar: CoefficientType, modular_arithmetic: &ModularArithmetic) {
...
impl ReductionMatrix {
    ...
    pub fn add_column(&mut self, column_index: IndexType, entries: Vec<Entry>) -> Option<(ValueType, ValueType)> {
            ...
            if let Some(&reducing_column_index) = self.pivot_to_column.get(&pivot) {
                ...
                if let Some(reducing_column) = self.columns.get(&reducing_column_index) {
                    ...
                    let scalar = self.modular_arithmetic.get_modulo(
                        ...
                        self.modular_arithmetic.inverse(reducing_pivot_entry.coefficient)
                    );
...
mod tests {
    ...
    fn test_sparse_column_operations() {
        let mut col1 = SparseColumn::from_entries(vec![
            Entry::new(0, 1),
            Entry::new(2, 1),
            Entry::new(4, 1),
        ]);
        
        let col2 = SparseColumn::from_entries(vec![
            Entry::new(1, 1),
            Entry::new(2, 1),
            Entry::new(3, 1),
        ]);
