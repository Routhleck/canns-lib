...
use crate::core::{
    ValueType, IndexType, CoefficientType, DiameterEntry, Entry, 
    BinomialCoeffTable, Result, RipserError, check_overflow
};
...
impl CombinatorialIndex {
    ...
    pub fn index_to_simplex(&self, mut index: IndexType, dimension: usize) -> Result<Vec<IndexType>> {
        if dimension > self.max_dimension {
            return Err(RipserError::InvalidDimension { dim: dimension });
        }
...
impl<'a, M: DistanceMatrix> Iterator for CoboundaryIterator<'a, M> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        while self.next_vertex < n {
            ...
            match self.combinatorial_index.simplex_to_index(&coboundary_vertices) {
                Ok(coboundary_index) => {
...
impl<'a> SparseCoboundaryIterator<'a> {
    ...
    fn compute_intersection(&mut self) {
        ...
        let first_neighbors: Vec<_> = self.neighbors[0].as_slice().iter()
            .map(|(v, _)| *v)
            .collect();
        ...
        'candidate: for candidate in first_neighbors {
            ...
            for neighbors in &self.neighbors[1..] {
                let found = neighbors.as_slice().iter()
                    .any(|(v, _)| *v == candidate);
                if !found {
                    continue 'candidate;
                }
...
impl<'a> Iterator for SparseCoboundaryIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let insert_pos = self.coboundary_vertices.binary_search(&v).unwrap_or_else(|x| x);
        ...
        for i in 0..self.coboundary_vertices.len() {
            for neighbors in &self.neighbors {
                ...
                if let Ok(pos) = vertex_neighbors.binary_search_by_key(&self.coboundary_vertices[i], |(v, _)| *v) {
        ...
        // Compute index and coefficient
        match self.combinatorial_index.simplex_to_index(&self.coboundary_vertices) {
            Ok(coboundary_index) => {
                let coefficient = if insert_pos % 2 == 0 { 1 } else { -1 };
                let coefficient = self.modular_arithmetic.get_modulo(coefficient);
...
impl UnionFind {
    ...
    pub fn union(&mut self, x: IndexType, y: IndexType, diameter: ValueType) -> Option<(ValueType, ValueType)> {
