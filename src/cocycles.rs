...
use crate::core::{
    ValueType, IndexType, CoefficientType, Entry, Cocycle, SimplexCoeff, 
    ModularArithmetic, Result, RipserError,
};
...
impl CocycleExtractor {
    ...
    pub fn extract_cocycle(
        ...
        for &entry in self.cocycle_column.entries() {
            let vertices = self.combinatorial_index.index_to_simplex(entry.index, dimension)?;
        ...
        Ok(Cocycle { dimension, simplices })
    }
                ...
                if let Some(&reducing_column_index) = pivot_to_column.get(&entry.index) {
                    if let Some(reducing_column) = stored_columns.get(&reducing_column_index) {
                        ...
                        let scalar = self.modular_arithmetic.get_modulo(
                            entry.coefficient * 
                            self.modular_arithmetic.inverse(self.get_pivot_coefficient(reducing_column, entry.index)?)
                        );
                        
                        // Scale and add the reducing column
                        for &reducing_entry in reducing_column.entries() {
                            let scaled_coeff = self.modular_arithmetic.get_modulo(reducing_entry.coefficient * scalar);
                            if scaled_coeff != 0 {
                                // Add this entry to the cocycle
                                let existing_pos = self.cocycle_column.entries().iter()
                                    .position(|e| e.index == reducing_entry.index);
                                
                                if let Some(pos) = existing_pos {
                                    // Combine coefficients
                                    let mut entries = self.cocycle_column.entries().to_vec();
                                    let new_coeff = self.modular_arithmetic.get_modulo(
                                        entries[pos].coefficient + scaled_coeff
                                    );
                                    ...
                                    // Add new entry
                                    self.cocycle_column.push(Entry::new(reducing_entry.index, scaled_coeff));
                                }
    ...
    fn get_pivot_coefficient(&self, column: &SparseColumn, pivot_index: IndexType) -> Result<CoefficientType> {
        column.entries().iter()
...
impl CocycleManager {
    ...
    pub fn add_cocycle(&mut self, cocycle: Cocycle) -> Result<()> {
        if cocycle.dimension > self.max_dimension {
            return Err(RipserError::InvalidDimension { dim: cocycle.dimension });
        }
...
impl Cocycle {
    ...
    pub fn add_simplex(&mut self, vertices: Vec<IndexType>, coefficient: CoefficientType) {
        self.simplices.push(SimplexCoeff::new(vertices, coefficient));
    }
